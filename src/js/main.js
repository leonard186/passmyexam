...
var controller = (function(model, view){
    ...
    var renderQuizOnClick = function() {
        /*
        for (var i=0; i<model.quizCollection.length; i++) {
            console.log(i +1);
            var index = i+1;
            var n = model.quizCollection[index];
            model.getID(`btn-${index}`).addEventListener('click', function() {
                initiateQuiz(quiz1);
                model.countDownTimer();
                model.getID('quiz-name').innerHTML += ` ${index}`;
                model.getID('finished-btn').innerHTML += ` ${index}`;
            });
